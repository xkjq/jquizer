            ...
            $(".emq-question").each(
                function(index, option) {
                    ...
                    if (correct_option == selected_option) {
                        $(option).append("<br/><span class='emq-answer-feedback correct'>Correct: <b>" + correct_option + "</b> is the right answer<br/>-----------<br/>" + feedback + "</span>");
                        correct.push("correct")
                        n_correct = n_correct + 1;
                    } else {
                        //$(option).addClass("incorrect");
                        $(option).append("<br/><span class='emq-answer-feedback incorrect'>Incorrect: <b>" + correct_option + "</b> is the right answer (you said <i>"+selected_option+"</i>)<br/>-----------<br/>" + feedback + "</span>");
...
//                });
            ...
            correct_answers = $("#answer").attr("data-answer").split(",");
            ...
            $("#answer-block").append($(document.createElement("span")).attr({
                    ...
                    }).text("("+Math.round(best_sim * 100)/100+")")).append($(document.createElement("a")).attr({
                    "href": "https://www.google.com/search?q="+replaced_lower_case_answer,
                    "target": "newtab",
                    "class": "google-answer",
                    }).text("G")).append($(document.createElement("a")).attr({
                    "href": "https://www.imaios.com/en/content/search?SearchText="+replaced_lower_case_answer,
                    "target": "newtab",
                    "class": "imaios-answer",
                    }).text("I")));
...
//                });
            ...
            if (best_sim < 1) {
                $.each(wordlist, function(key, value) {
                    if (((best_answer.toLowerCase().indexOf(key) != -1) && (a.toLowerCase().indexOf(value) != -1)) && (best_answer.toLowerCase().indexOf(value) == -1)) {
                        best_sim = -1
                    }
                    ...
                    if (best_sim < 0) {
                        ...
                        $("#answer").append($(document.createElement("span")).attr({
                            "id" : "unique-words",
                        }).text(key+" != "+value+": "));
        ...
        case "label":
            ...
            $("#answer-list li").each(function(index, option) {
                ...
                a = input_box.val();
                input_box.replaceWith($(document.createElement("span")).attr({
                ...
                if (a.toLowerCase() == correct_answer.toLowerCase()) {
                    ...
                    $(option).append($(document.createElement("span")).attr({
                    "class": "label-correct-answer-text",
                    }).text(correct_answer));
                ...
                replaced_lower_case_answer = correct_answer.toLowerCase().replace("left", "").replace("right", "")

                $(option).append($(document.createElement("a")).attr({
                    "href": "https://www.google.com/search?q="+replaced_lower_case_answer,
                    "target": "newtab",
                    "class": "google-answer",
                    }).text("G")).append($(document.createElement("a")).attr({
        ...
        case "tf":
            ...
            $(".tf-answer-block li").each(function (index, option) {
                ...
                if ($(option).hasClass(answer)) {
                    ...
                    correct.push("correct");
                    n_correct = n_correct + 1;
                    $(option).append("<br/><span class='emq-answer-feedback'>"+ answer_option + feedback + "</span>");
                } else {
                    $(option).addClass("incorrect");
                    correct.push("incorrect");
                    $(option).append("<br/><span class='emq-answer-feedback'>"+ answer_option + feedback + "</span>");
